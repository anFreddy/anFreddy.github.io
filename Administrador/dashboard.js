...
async function cargarAsistencia() {
  ...
  try {
    ...
    const asistencia = await apiFetch(
      `alumnos/asistenciaTotal?fecha=${fecha}&seccion=${seccion}`,
    );
...
function exportarExcel() {
  ...
  const headerRow = tablaClon.querySelector("thead tr");

  const thDetalles = headerRow.lastElementChild;

  thDetalles.remove();

  ["Entrada 1", "Salida 1", "Entrada 2", "Salida 2"].forEach((texto) => {
  ...
  filas.forEach((fila) => {
    ...
    badges.forEach((badge) => {
      valores.push(badge.textContent.trim());
    });
